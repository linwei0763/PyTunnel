...
class Ring():
    ...
    def compute_d_seg_lin(self):
        ...
        bounds = []
        for i in range(self.num_seg):
            bounds.append((-0.025, 0.025))
        for i in range(self.num_seg):
            bounds.append((-np.pi / 72, np.pi / 72))
        for i in range(3):
        ...
        delta_theta = param[2 * self.num_seg + 3]
    ...
    def fit_seg(param, num_seg, r, length, width, angles_b, angles_m, angles_f, xyz_p, label):
        ...
        delta_z = param[2 * num_seg + 2]
        delta_theta = param[2 * num_seg + 3]
